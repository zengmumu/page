...
if ($action == 'import') { //导入XLS
	...
	for ($i=2; $i<=$xls->sheets[0]['numRows']; $i++) {
			...
			$body = $xls->sheets[0]['cells'][$i][11];
			$difficulty = $xls->sheets[0]['cells'][$i][12];
			$vocation = $xls->sheets[0]['cells'][$i][13];
			$data_values .= "('$id','$typeid','$title',30),";
			$data_values2 .= "('$id','$typeid','$subtitle','$type','$optiona','$optionb','$optionc','$optiond','$answer','$body','$difficulty','$vocation'),";
...
//			    
//			        //保存到附近加表
				    $query = "insert into  qkt_addonarticle30 (aid,typeid,subtitle,type,optiona,optionb,optionc,optiond,answer,'body','difficulty','vocation') values  $data_values2";
//				    echo $query;
