...
function ch_softlinks2($fvalue, &$ctag, &$refObj, $fname='', $downloadpage=false)
{
    ...
    $downlinks = '';
	$v=array();
	$getvideo=$ctag->CAttribute->Items['getvideo'];
	$fid=$ctag->CAttribute->Items['fid'];
    ...
    {
		if($getvideo==1){
			 $tempStr = GetSysTemplets("video_links.htm");
			}else{
...
$str=ch_softlinks_all($fvalue, $ctag, $refObj, $row);
die($str);
//return $str;
...
if($getvideo==1){
    ...
    foreach($dtp->CTags as $ctag)
    {
		...
		if($times==1){ $temp = str_replace("~server~'",$serverName."','default':true",$temp);
		}else{
    ...
    if($row['ismoresite']==1 && $row['moresitedo']==1 && trim($row['sites'])!='' && isset($firstLink))
    {
        ...
        {$times++;
			...
			$temp = str_replace("~server~'",$serverName."','default':true",$temp);
...
$downlinks= substr($downlinks,0,strlen($downlinks)-1); 
//die($downlinks);
    return $downlinks;
