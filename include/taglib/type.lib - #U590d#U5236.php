...
//var_dump($refObj->Fields);
	//die("----------");
	/*le=2 le=3 都是二级栏目的全部 le=24注意24是指顶级 在前面 le=1 是顶级栏目*/
  ...
  $myreid=$refObj->Fields[reid];
	...
	if($le==24){
    $row = $dsql->GetOne("SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath 
                          FROM `#@__arctype` WHERE id=24 ");
	}
	if($refObj->Fields[reid]==$refObj->Fields[topid]&&($le==2||le==3)){
...
id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath 
	...
	if($id==$refObj->Fields[id]||($refObj->Fields[reid]==$refObj->Fields[topid]&&($le=='2'||$le=='3'))||$le==1&&$refObj->Fields[id]==12||$le==24&&$refObj->Fields[id]==24){
		//var_dump($id);
		//die("|--------------");
