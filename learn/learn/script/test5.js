...
var swiper = new Swiper('.swiper-container', {
		        ...
		        onInit: function(swiper){
		        	// alert(api.winName);
		        	// alert(storage.getItem(api.winName,"lastslide"));
		        	if(storage.getItem(api.winName,"lastslide")){
		        		
		        		var lastslide=storage.getItem(api.winName,"lastslide");
		         	...
		         	/*if(localStorage.getItem(api.winName)){
		         		var winStorage=JSON.parse(localStorage.getItem(api.winName));
		         		  
		         		  if(winStorage.lastslide){
		         		  	 swiper.slideTo(winStorage.lastslide);  
		         		  }else{
...
swiper.on('SlideChangeEnd', function(swiper){
        ...
        storage.setItem(api.winName,"lastslide",index);
      /*  
        if(localStorage.getItem(api.winName)){
        	var winStorage=JSON.parse(localStorage.getItem(api.winName));
        	    winStorage.lastslide=index;
          ...
          alert(winStorage);
        window.localStorage.setItem(api.winName,winStorage);*/
        })
//////////////////////////////////////////返回上一页
$(".back").click(function(){
...
storage.pushItem("wrong","wrong",parent.data[swiper.activeIndex]);
swiper.slides[swiper.activeIndex].oldCheck=false;
...
$(".collection").click(function(){
	...
	storage.pushItem("favorite","favorite",parent.data[swiper.activeIndex]);
...
$(".delete").click(function(){
	// alert("删除");
	// alert(api.winName);
	if(  storage.removeData(api.winName,api.winName,parent.data[swiper.activeIndex])){
...
swiper.on('SlideChangeStart', function(swiper){
 ...
 /* var favorite=JSON.parse(localStorage.getItem("favorite"))||{};
  ...
  this.slider=swiper.slides[swiper.activeIndex];
   ...
   if(storage.hasData("favorite","favorite",parent.data[swiper.activeIndex])){
   	    $(".collection").addClass("active");
...
var storage={
	...
	,
	pushItem:function(name,key,obj){
		if(localStorage.getItem(name)){
				var winStorage=JSON.parse(localStorage.getItem(name));				 
		   ...
		   winStorage[key].push(obj);
		     winStorage=JSON.stringify(winStorage);
		      window.localStorage.setItem(name,winStorage);
		      // alert(winStorage);

	},//pushItem
	hasData:function(name,key,obj){
		
		if(localStorage.getItem(name)){
			var winStorage=JSON.parse(localStorage.getItem(name));	
			if(winStorage[key]){
				for(var k in winStorage[key]){
					if(winStorage[key][k].id==obj.id){
						return true;
					}
				}
				return false;
	...
	removeData:function(name,key,obj){
		if(localStorage.getItem(name)){
			var winStorage=JSON.parse(localStorage.getItem(name));	
			// alert(localStorage.getItem(name));
			if(winStorage[key]){

				for(var i=0;i<winStorage[key].length;i++){
					if(winStorage[key][i].id==obj.id){
						// alert( winStorage[key])
					   winStorage[key].splice(i,1);
					   return true;
					   // alert(i);
					   break;
					}
